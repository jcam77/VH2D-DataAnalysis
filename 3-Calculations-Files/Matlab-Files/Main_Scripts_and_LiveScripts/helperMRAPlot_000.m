function helperMRAPlot_000(data,mra,time,type,titlestr,PeakFrequencies,freqPassbands,varargin)
...
for kk = 1:Numplots
    ...
    if kk < Numplots
        plot(ax(kk),time,mra(kk,:),'Color',LUBlue);
        ...
        if ~isempty(colorAx) && ismember(kk,colorAx)
            ...
            ax(kk).Color = LUCopper_Light;
        end
    else
        plot(ax(kk),time,data,'Color',LUBlue);
        ax(kk).XLim = [min(time) max(time)];
        minval = min(data);
        maxval = max(data);
        ax(kk).YLim = [minval maxval];
    ...
    if kk <= Numplots-2
        if strcmpi(type,'wavelet')
            ...
            ylabel(ax(kk),['IMF-' num2str(kk) ' Pk-' num2str(PeakFrequencies(kk), '%.1f') 'Hz' ' - ' ' (' num2str(freqPassbands(kk,1), '%.0f') ' - ' num2str(freqPassbands(kk,2), '%.0f') ') Hz']);
        end
