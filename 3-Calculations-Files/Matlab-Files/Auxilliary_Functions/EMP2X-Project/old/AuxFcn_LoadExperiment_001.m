function tests = AuxFcn_LoadExperiment_001(cfg)
% AuxFcn_LoadExperiment_001 (NO MERGE / NO RESAMPLING)
...
%
% Output:
%   tests(i).streams(k) : struct with fields:
%       .id   : DAQ id string
...
% tests = AuxFcn_LoadExperiment_001(cfg);
...
for i = 1:nTests
    tests(i).streams = repmat(struct('id',"",'data',[]), numel(daqs), 1);
...
if fileExists
    try
        switch fmt
            case "txt"
                daqData = AuxFcn_ReadDAQ_TXT_000(char(thisFileResolved), ...
                    'DaqId', id, ...
                    'Units', daqs(k).units, ...
                    'ChannelType', daqs(k).channelType);

            case "tpc5"
                daqData = AuxFcn_ReadTPC5_000(char(thisFileResolved), ...
                    'DaqId', id, ...
    ...
    warning('Test %d: File not found for %s -> %s (pwd=%s)', i, id, thisFile, pwd);
        ...
        tests(i).streams(k).id   = id;
        tests(i).streams(k).data = daqData;
