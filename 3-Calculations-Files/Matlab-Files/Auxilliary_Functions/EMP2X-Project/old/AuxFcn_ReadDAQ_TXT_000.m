...
% daq1 = AuxFcn_ReadDAQ_TXT_000('Problem grounding.txt', 'DaqId',"DAQ1");
...
%   daq1 = AuxFcn_ReadDAQ_TXT_000('Test_001_DAQ1.txt', ...
...
p = inputParser;
p.addRequired('rawFileName', @(x) ischar(x) || isstring(x));
...
p.parse(rawFileName, varargin{:});
opt = p.Results;

rawFileName = string(rawFileName);
thisFile = which(rawFileName);
if isempty(thisFile)
    thisFile = char(rawFileName);
    assert(exist(thisFile,'file')==2, 'Raw data file not found: %s', rawFileName);
end
...
% ---------- Outputs ----------
daqData = struct();
daqData.name = string(rawFileName);
daqData.file = string(thisFile);
...
if numel(varNames) >= (1+nChannels)
    for k = 1:nChannels
        chNames(k) = string(varNames(k+1));
    end
end
